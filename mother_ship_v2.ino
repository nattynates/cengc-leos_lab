...
Serial.print("Colour: "); Serial.println(colour_int);

if (colour_int == 0) {
  // if white drive backwards
  Backward(drivetime);
  // then rotate a bit
  TurnRight(drivetime/2);
} else if (colour_int == 100) {
  // If black
} else {
  // if its a colour, RANDOM!
...
// After this many iterations just kill self and hope its on the end square
if (iter > 500000) {
  while(1) {
...
int determine_colour(uint16_t R, uint16_t G, uint16_t B) {
  ...
  // White
  else if (R > 12000 && G > 15000 && B > 14000) {
    return 0;
  }
  // yellow
